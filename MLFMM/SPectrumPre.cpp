...
void SpectrumPre::Get_Spectrum()
{
	...
	for (int i = 0; i < level_num; ++i) {
		int L = GetL(length * float(1 << i) * sqrt(3.0), wavenumber);
		int theta_num = GetThetaNum(L);//1.5, 
		int phi_num = GetPhiNum(L);

		vector<float> thetas_level(theta_num);
		...
		std::vector<Eigen::Vector3f> k_unit;
		std::vector<float> weights_level;
		float theta_dif = pi / (theta_num - 1);
		float phi_dif = 2 * pi / phi_num;
		if (mpipre.GetRank() == 0) cout << "theta数量" << theta_num << endl;
		int process_num = mpipre.GetProcessNum(i);
		int index = mpipre.GetProcessIndex(i);
		float theta_st = pi / process_num * index, theta_end = pi / process_num * (index + 1);
		...
		if (i > 0) {
			...
			float theta_st_son = pi / son_process_num * son_index, theta_end_son = pi / son_process_num * (son_index + 1);
	...
	int L = GetLH(length * sqrt(3.0), wavenumber);
	...
	thetas_num.push_back(theta_num);
	phis_num.push_back(phi_num);
	std::vector<Eigen::Vector3f> k_need;
	k_need.reserve(theta_num * phi_num);
	TCtoS.reserve(theta_num * phi_num);
	float phi_dif = 2 * pi / phi_num;
	for (int j = 0; j < theta_num; j++) {
		for (int k = 0; k < phi_num; k++) {
			float ct = cos(thetas_level[j]);
			float st = sin(thetas_level[j]);
			float cp = cos(k * phi_dif);
			float sp = sin(k * phi_dif);
			k_need.push_back(wavenumber * Vector3f{ st * cp, st * sp, ct });
			Matrix<float, 2, 3> T;
			T << ct * cp, ct* sp, -st,
				-sp, cp, 0;
			TCtoS.push_back(T);
...
{
	float fa_phi_dif = 2 * pi / fa_phi_num;
	float son_phi_dif = 2 * pi / son_phi_num;
	...
	for (int i = u_theta_st; i < u_theta_end; i++) {
		...
		for (int j = 0; j < son_phi_num; j++) {
			...
			//int mbegin = max(m0 - p + 1, d_theta_st), mend = min(m0 + p, d_theta_end);
			int mbegin = m0 - p + 1, mend = m0 + p;
			for (int m = mbegin; m <= mend; m++) {
				float mtheta = GetTheta(m);
				int son_index = GetIndex(m, j) - son_shift;
				float coef = IsVirtual(m);
				for (int a = m0 - p + 1; a <= m0 + p; a++) {
					if (a == m) continue;
					float atheta = GetTheta(a);
